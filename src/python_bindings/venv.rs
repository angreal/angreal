...
impl VirtualEnv {
    ...
    fn __new__(path: Option<PyObject>, python: Option<&str>, requirements: Option<PyObject>, now: bool) -> PyResult<Self> {
        Python::with_gil(|py| {
            ...
            let path_str = if let Some(path_obj) = path {
                if let Ok(s) = path_obj.extract::<String>(py) {
                    ...
                    path_obj.call_method0(py, "__str__")?.extract::<String>(py)?
                }
            } else {
                ".venv".to_string()
            };
            ...
            let path_buf = if path_str.starts_with('/') || path_str.starts_with('~') {
                ...
                std::env::current_dir()
                    .map_err(|e| PyErr::new::<pyo3::exceptions::PyIOError, _>(format!("Cannot get current directory: {}", e)))?
                    .join(&path_str)
            };
            ...
            let resolved_path = {
                ...
                let resolved_str = resolved_py_path.call_method0("__str__")?.extract::<String>()?;
    ...
    fn activate(&mut self) -> PyResult<()> {
        ...
        Python::with_gil(|py| {
            if !self.exists(py)? {
                return Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(
                    format!("Virtual environment at {} does not exist", self.path.display())
                ));
            }
            ...
            let site_packages = if cfg!(windows) {
                ...
                self.path.join("lib").join(format!("python{}.{}", 
                    py.version_info().major, py.version_info().minor)).join("site-packages")
            };
    ...
    fn install(&self, packages: PyObject) -> PyResult<()> {
        Python::with_gil(|py| {
            ...
            if let Ok(package_str) = packages.extract::<String>(py) {
                ...
                if package_str.ends_with(".txt") {
                    ...
                    let output = Command::new(&pip_exe)
                        ...
                        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(
                            format!("Failed to install requirements: {}", e)
                        ))?;
                    
                    if !output.status.success() {
                        return Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(
                            format!("pip install failed: {}", String::from_utf8_lossy(&output.stderr))
                        ));
                    }
                } else {
                    // Single package
                ...
                let package_str = packages.call_method0(py, "__str__")?.extract::<String>(py)?;
                let output = Command::new(&pip_exe)
                    .arg("install")
                    .args(["-r", &package_str])
                    .output()
    ...
    fn discover_available_pythons(_cls: &PyType) -> PyResult<Vec<(String, String)>> {
        ...
        Ok(vec![
            ("cpython-3.11.10".to_string(), "/usr/bin/python3.11".to_string()),
            ("cpython-3.12.9".to_string(), "/usr/bin/python3.12".to_string()),
        ])
    }
...
impl VenvRequiredWrapper {
    ...
    fn __call__(&self, args: &pyo3::types::PyTuple, kwargs: Option<&pyo3::types::PyDict>) -> PyResult<PyObject> {
        Python::with_gil(|py| {
