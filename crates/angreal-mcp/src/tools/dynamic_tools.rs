...
pub fn discover_angreal_commands() -> Vec<Tool> {
    ...
    match try_discover_from_registry() {
        Ok(tools) => {
            debug!("Successfully discovered {} tools from ANGREAL_TASKS registry", tools.len());
            tools
        }
        Err(e) => {
...
fn try_discover_from_registry() -> Result<Vec<Tool>, Box<dyn std::error::Error>> {
    ...
    for (path, command) in tasks.iter() {
        ...
        let tool_name = format!("angreal_{}", path.replace(".", "_").replace(" ", "_").replace("-", "_"));
        debug!("Converting command '{}' (path: {}) to MCP tool '{}'", command.name, path, tool_name);
    ...
    debug!("Successfully discovered {} tools from ANGREAL_TASKS registry", tools.len());
    for tool in &tools {
        debug!("  - {}: {}", tool.name, tool.description.as_deref().unwrap_or("No description"));
    }
        ...
        Ok(project_path) => {
            ...
            match angreal::utils::get_task_files(project_path.join(".angreal")) {
                Ok(task_files) => {
                    ...
                    task_files.iter().enumerate().map(|(i, file)| {
                        let fallback_name = format!("task_{}", i);
                        let task_name = file.file_stem()
...
fn generate_enhanced_description(command: &angreal::task::AngrealCommand) -> String {
    let mut description = command.about.clone().unwrap_or_else(|| "Angreal command".to_string());
...
fn generate_command_schema(_command: &angreal::task::AngrealCommand, command_path: &str) -> Result<ToolInputSchema, Box<dyn std::error::Error>> {
